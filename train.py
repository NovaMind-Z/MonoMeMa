...
def get_training_roidb(imdb):
    """Returns a roidb (Region of Interest database) for use in training.
       输入imdb是 <lib.datasets.pascal_voc.pascal_voc object at 0x0000027F1E7A07B8>
       """
    if True:  # 图片翻转
...
def combined_roidb(imdb_names):
    """
    Combine multiple roidbs  imdb_names ='kitti_train'  voc_2007_trainval
    """
    def get_roidb(imdb_name):
        imdb = get_imdb(imdb_name)  # imdb就是一个实例变量了。可以自由地给一个实例变量绑定属性，比如，给实例绑定一个name属性
    ...
    if len(roidbs) > 1:
        for r in roidbs[1:]:
            roidb.extend(r)  # extend() 向列表尾部追加一个列表，将列表中的每个元素都追加进来，在原有列表上增加
...
class Train:
    def __init__(self):
        ...
        # self.imdb, self.roidb = combined_roidb("voc_2007_trainval")kitti_2012_train"
        self.imdb, self.roidb = combined_roidb("kitti_2012_trainval")
    ...
    def train(self):
        ...
        with sess.graph.as_default():
            ...
            tf.set_random_seed(cfg.FLAGS.rng_seed)  # 设置图级随机seed 如果设置了图级seed，但操作seed没有设置：系统确定性地选择与图级seed一起的操作seed，以便获得唯一的随机序列。
        ...
        # saver.restore(sess, 'default/kitti_2012_trainval/model/vgg16_faster_rcnn_iter_145000.ckpt')
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loos_dis, total_loss = self.net.train_step(sess, blobs, train_op)
            timer.toc()
            iter += 1
            ...
            if iter % (cfg.FLAGS.display) == 0:
                print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                      '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> loss_dis: %.6f\n ' % \
                      (iter, cfg.FLAGS.max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loos_dis))
