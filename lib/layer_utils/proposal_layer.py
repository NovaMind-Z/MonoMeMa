...
def proposal_layer(rpn_cls_prob, rpn_bbox_pred, im_info, cfg_key, _feat_stride, anchors, anchors_dis, num_anchors):
    ...
    blob = np.hstack((batch_inds, proposals.astype(np.float32, copy=False), proposals_dis.astype(np.float32, copy=False)))
    return blob, scores
