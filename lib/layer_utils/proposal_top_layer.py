...
def proposal_top_layer(rpn_cls_prob, rpn_bbox_pred, im_info, _feat_stride, anchors, anchors_dis,  num_anchors):
    ...
    anchors_dis = anchors_dis[top_inds, :]
    # Convert anchors into proposals via bbox transformations
    proposals = bbox_transform_inv(anchors, rpn_bbox_pred)
    # print("proposals", proposals.shape, anchors_dis.shape)
    # Clip predicted boxes to image
    proposals = clip_boxes(proposals, im_info[:2])
    # print("good", proposals.shape, anchors_dis.shape)
    # Output rois blob
