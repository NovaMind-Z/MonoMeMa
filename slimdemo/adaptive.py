...
class Adaptive_Network(LSTM, CITY):
    ...
    def data_process(self, fc, kitti_memory_0323):
        self.test_data = [fc, kitti_memory_0323]
    ...
    def run_test(self, sess):
        queary_test = iter(self.test_data[0]) # city_depth_label
        self.memory_store = self.test_data[1]
        count = 0
        acc = 0
        pre_dep = []
        rea_dep = []
        ...
        while True:
            try:
                queary_x_test = next(queary_test)
                queary_x_test = np.reshape(queary_x_test, (1, -1))
                # print("name", kiname, queary_y_test)
                ###### add by zjk
                # find neighbor
                neighbor_vector, neighbor_label, neighbor_key, neighbor_box = self.KNN(self.memory_store, queary_x_test, self.k)
                ###### add by zjk
                if self.depth == 1 and len(neighbor_label) == 2:
                    neighbor_label = neighbor_label[1]
                ###### add by zjk
        ...
        return np.reshape(pre_dep, (-1, 1)), neighbor_key, neighbor_box, neighbor_label
    '''
    city kitti_memory
    0 train:
    100m: scale = 1.5
    ...
    def KNN(self, memory_store, queays, TopK):
        ...
        for i, res in enumerate(sort_dis):
            memory_x[TopK-1-i, :] = memory_store[res[0]][0]
            ###### add by zjk
            ...
            memory_key.insert(0, memory_store[res[0]][3])
            memory_box.insert(0, memory_store[res[0]][4])
        return memory_x, memory_y, memory_key, memory_box
